...
# ======================================
# WEATHER FUNCTION (TODAY + TOMORROW)
# ======================================
...
with col1:
    st.subheader("Today")
    st.write("Temperature:", today_temp, "°C")
    st.write("Rain:", "Yes" if today_rain == 1 else "No")

with col2:
    st.subheader("Tomorrow")
    st.write("Avg Temperature:", round(tomorrow_temp,2) if tomorrow_temp else "N/A", "°C")
...
if today_temp is not None:
    today_input = pd.DataFrame([[sales, today_temp, today_rain, event]], columns=features)
...
# Tomorrow Prediction
if tomorrow_temp:
    tomorrow_input = pd.DataFrame([[sales, tomorrow_temp, tomorrow_rain, event]], columns=features)
...
report_type = st.selectbox("Select Report Type", ["Daily (Current Month)", "Weekly (Current Month)", "Yearly"])
...
if report_type == "Daily (Current Month)":
    daily = current_month_data.groupby(current_month_data["date"].dt.date)["food_waste"].sum()
    st.line_chart(daily)
