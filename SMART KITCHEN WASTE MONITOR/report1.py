...
# MODEL TRAINING
# ======================================
features = ["quantity", "temperature", "rain", "event"]
...
with col1:
    st.subheader("Today")
    st.write("Temperature:", today_temp if today_temp else "N/A", "°C")
...
with col2:
    ...
    st.write(
        "Avg Temperature:",
        round(tomorrow_temp, 2) if tomorrow_temp else "N/A",
        "°C"
    )
...
quantity = st.slider("Expected Quantity Today", 200, 600, 400)
event = st.selectbox("Local Event Today?", [0, 1])

if today_temp is not None:
    today_input = pd.DataFrame(
        [[quantity, today_temp, today_rain, event]],
        columns=features
    )

    predicted_today = model.predict(today_input)[0]
    recommended_today = quantity - predicted_today
    ...
    st.write("Recommended Quantity:", round(recommended_today, 2))

if tomorrow_temp is not None:
    tomorrow_input = pd.DataFrame(
        [[quantity, tomorrow_temp, tomorrow_rain, event]],
        columns=features
    )
...
ax.set_title("Quantity vs Food Waste Relationship")
st.pyplot(fig)
